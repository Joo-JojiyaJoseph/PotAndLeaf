...
class UserController extends Controller
{
    ...
    /** Replace the user's roles for this company with (at most) one role. */
    private function syncCompanyRole(User $user, int|string $companyId, ?string $roleId): void
    {
        $companyRoleIds = Role::forCompany($companyId)->pluck('id')->all();
    ...
    {
        $user->load(['roles' => fn ($q) => $q->where('company_id', $companyId)]);
