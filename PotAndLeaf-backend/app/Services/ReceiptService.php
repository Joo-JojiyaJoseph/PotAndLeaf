...
class ReceiptService
{
    ...
    public function receivables(int|string $companyId, ?string $customerId = null): array
    {
        return Sale::query()
            ...
            ->get()
            ->map(function (Sale $s) {
                $credit = round((float) $s->grand_total - (float) $s->amount_paid, 2);
                $received = (float) ($s->received_sum ?? 0);
                $balance = round($credit - $received, 2);
                ...
                return [
                    ...
                    'invoice_total' => (float) $s->grand_total,
            ...
            ->filter(fn ($r) => $r['credit_amount'] > 0.005)
            ->values()
            ->all();
    }
