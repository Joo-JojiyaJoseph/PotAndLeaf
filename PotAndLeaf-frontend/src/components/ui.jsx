...
export function CardHeader({ title, actions }) {
  return (
    <div className="flex items-center justify-between border-b border-line bg-[#FAFBFA] px-4 py-3">
...
const badgeTones = {
  ...
  inactive: 'bg-[#F1F5F3] text-muted',
  draft: 'bg-[#F1F5F3] text-muted',
  blocked: 'bg-danger-soft text-danger',
  rejected: 'bg-danger-soft text-danger',
  warning: 'bg-amber-soft text-amber',
  pending: 'bg-[#FDEEDC] text-[#C1721F]',
...
export function Modal({ open, onClose, title, children, footer }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/30 p-0 sm:items-center sm:p-4">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-t-[20px] bg-surface shadow-pop sm:rounded-[20px]">
