...
const TABS = [{ value: 'payouts', label: 'Payouts' }, { value: 'rules', label: 'Rules' }];
...
function RuleModal({ open, onClose, staff, editing }) {
  ...
  const [form, setForm] = useState({ user_id: '', base_percent: '', monthly_target: '', target_bonus: '', is_active: true, notes: '' });
  const [errors, setErrors] = useState({});
  const [applied, setApplied] = useState(null);

  if (open && editing && applied !== editing.id) {
    setForm({ user_id: String(editing.user_id), base_percent: String(editing.base_percent), monthly_target: String(editing.monthly_target), target_bonus: String(editing.target_bonus), is_active: editing.is_active, notes: editing.notes ?? '' });
    setApplied(editing.id);
  }

  const saveM = useMutation({
    mutationFn: () => api.post('/commission/rules', {
      user_id: Number(form.user_id), base_percent: Number(form.base_percent) || 0,
      monthly_target: Number(form.monthly_target) || 0, target_bonus: Number(form.target_bonus) || 0,
  ...
  function handleClose() { setForm({ user_id: '', base_percent: '', monthly_target: '', target_bonus: '', is_active: true, notes: '' }); setErrors({}); setApplied(null); onClose(); }
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <Field label="Base % of sales" required error={err('base_percent')}><Input type="number" step="0.001" value={form.base_percent} onChange={set('base_percent')} /></Field>
    ...
    <div className="space-y-5 p-4 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted">Staff commission on billed sales, with monthly-target bonuses.</p>
        </div>
                ...
                <tbody>
                  {rules.map((r) => (
                    <tr key={r.id} className="border-b border-line/60 last:border-0 hover:bg-sidebar/60">
                      ...
                      <td className="tnum px-4 py-2.5 text-right">{r.base_percent}%</td>
