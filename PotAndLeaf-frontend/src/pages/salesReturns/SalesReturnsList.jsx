...
export default function SalesReturnsList() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['sales-returns', activeCompany?.id, status],
    queryFn: () => api.get('/sales-returns', { params: { status } }).then((r) => r.data),
    ...
    <div className="space-y-5 p-4 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted">Credit notes to customers. Confirming restores stock and reduces receivables.</p>
        </div>
