...
export default function CustomersList() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['customers', activeCompany?.id, debounced, type],
    queryFn: () => api.get('/customers', { params: { search: debounced, type } }).then((r) => r.data),
  ...
  const deleteM = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['customers'] }); setDeleting(null); },
  });
  ...
  return (
    <div className="space-y-5 p-4 sm:p-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <select value={type} onChange={(e) => setType(e.target.value)} className={selectCls + ' max-w-[160px]'}>
              ...
              <tbody>
                {rows.map((c) => (
                  <tr key={c.id} className="border-b border-line/60 last:border-0 hover:bg-sidebar/60">
                    ...
                    <td className="px-4 py-2.5"><Badge tone={c.status === 'active' ? 'active' : c.status === 'blocked' ? 'blocked' : 'inactive'}>{c.status}</Badge></td>
                    <td className="px-4 py-2.5">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {can('customers.delete') && <button onClick={() => setDeleting(c)} className="rounded-lg p-1.5 text-muted hover:bg-paper hover:text-danger" aria-label="Delete"><TrashIcon className="size-4" /></button>}
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      >
        <p className="text-sm text-muted">Delete <span className="font-medium text-ink">{deleting?.name}</span>? This is a soft delete.</p>
      </Modal>
    </div>
  );
}
